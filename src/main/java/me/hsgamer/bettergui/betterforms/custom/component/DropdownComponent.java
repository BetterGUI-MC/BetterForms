...
public class DropdownComponent extends ValueCustomFormComponent {
    private final Map<CustomForm, List<String>> optionsMap = new ConcurrentHashMap<>();
    private final String text;
    private final List<String> options;
    private final String defaultOption;

    public DropdownComponent(CustomFormComponentBuilder.Input input) {
        super(input);
        this.text = Optional.ofNullable(MapUtils.getIfFound(input.options, "text"))
                .map(Object::toString)
                .orElse("");
        this.options = Optional.ofNullable(MapUtils.getIfFound(input.options, "option", "options"))
    ...
    protected String getValueFromResponse(CustomForm customForm, CustomFormResponse response) {
        return optionsMap.get(customForm).get(response.asDropdown());
    }
